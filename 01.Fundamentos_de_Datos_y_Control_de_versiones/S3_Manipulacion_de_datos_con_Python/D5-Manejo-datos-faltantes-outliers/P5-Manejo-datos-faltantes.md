...
Se vuelve a aplicar la función ``detectar_outliers_iqr(data, columna) `` pero ahora a la columna ya transformada. Como log reduce la escala, es esperable que más valores caigan dentro del rango intercuartílico "permitido" y por lo tanto que haya menos outliers.
